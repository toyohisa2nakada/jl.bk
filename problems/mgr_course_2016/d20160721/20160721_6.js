new function(){
	...
	var self_n = problems.problems.push([
		function(){
			...
			plib.setExpectedOutputs(["出力<br>8<br>1","出力<br>4<br>-6","出力<br>1<br>1","出力<br>5000<br>-5000"]);
			window.exec({module:"input",command:"setInitial",params:{
				pnumber:self_n,
				value:[
					{name:"a",initValue:["[1,8,2,3,5]","[2,4,3,-6,1,3]","[1]","[-5000,5000]"]},
				],
			...
			window.exec({module:"code",command:"setInitialText",params:{
				setEditable:[[{line:7,ch:0},{line:8,ch:0}]],
				text:"\
\/\/ 配列変数aの数値の中で、最大値と最小値を出力するプログラムを作成してください。\n\
\/\/ 出力は、最大値、最小値の順番で出力してください。\n\
\/\/ \n\
\/\/ print(max);は、maxという名前の変数の中身を出力します。\n\
\/\/ var max; は、maxという名前の変数を作成します。\n\
\/\/ max = a[0];は、変数maxに配列変数aの最初の数字を代入します。\n\
\/\/ \n\
\n\
"}});
