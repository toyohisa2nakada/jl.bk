new function(){
	...
	var self_n = problems.problems.push([
		function(){
			...
			plib.setExpectedOutputs(["出力<br>1の個数 1","出力<br>1の個数 2","出力<br>1の個数 4"]);
			window.exec({module:"input",command:"setInitial",params:{
				pnumber:scriptName,
				message:plib.getExpectedOutputs(),
				value:[
					{name:"a",initValue:["[1,2,3]","[-1,1,2,1,-2]","[1,1,1,1]"]},
				],
			}});
			...
			window.exec({module:"code",command:"setInitialText",params:{
				...
				text:"\
\/\/ 変数aの中にある1の数字の個数を出力してください。\n\
\/\/ print(\"1の個数 \"+n);とすると、1の個数という文字の後に変数nの値を出力します。\n\
\/\/ n += 1;は、nの値に1を足します。\n\
\n\
\n\
\n\
