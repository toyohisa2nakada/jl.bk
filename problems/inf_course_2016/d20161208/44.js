new function(){
	...
	var self_n = problems.problems.push([
		function(){
			...
			plib.setExpectedOutputs(["出力<br>平均 3.5<br>分散 1.25","出力<br>平均 3.25<br>分散 10.6875","出力<br>平均 5<br>分散 15.2"]);
			window.exec({module:"input",command:"setInitial",params:{
				pnumber:scriptName,
				message:plib.getExpectedOutputs(),
				value:[
					{name:"a",initValue:["[2,3,4,5]","[4,-1,2,8]","[4,9,5,-3,9,10,1,4,3,8]"]},
				],
			}});
			window.exec({module:"watch",command:"addValue",params:{name:"a"}});
			window.exec({module:"watch",command:"addValue",params:{name:"i"}});
			window.exec({module:"watch",command:"addValue",params:{name:"u"}});
			window.exec({module:"watch",command:"addValue",params:{name:"v"}});
			window.exec({module:"output",command:"setInitial",params:{pnumber:scriptName}});
			window.exec({module:"scripts",command:"setScriptName",params:scriptName});
			window.exec({module:"code",command:"setInitialText",params:{
				setEditable:[[{line:5,ch:0},{line:8,ch:0}]],
				text:"\
\/\/ 変数aの値の平均と分散を求めてください。\n\
\/\/ print(\"平均 \"+u);とすると、平均という文字の後ろにuの値を出力します。\n\
\/\/ u += 2;は、uの値に2を足します。u /= 5;は、uの値にu/5の値を代入します。\n\
\/\/ a.lengthは、aの値の個数を表します。\n\
\/\/ Math.pow(p,2)は、pの2乗を計算します。\n\
\n\
\n\
\n\
"}});
