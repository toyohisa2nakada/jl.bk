...
//			window.exec({module:"input",command:"setReadOnly"});
		...
		function(){
			...
			w.$("#input").instruct({
				string:"ここでは、初期データを変数にセットとします。変数は、記号で表され、<br>例えばこの場合は、aという名前の変数が1つあるとなります。",
				closeButton:true,
		...
		function(){
			...
			w.$("#run").instruct({
				string:"この下ではプログラムを書くことができます。実行ボタンを押すと、プログラムは実行されます。",
				closeButton:true,
				closedHandler:function(){
					problems.next();
				},
		...
		function(){
			...
			w.$("#panel").instruct({
				string:"実行した結果は、ここの表示されます。<br>printという命令が、ここへ文字や数字を表示します。",
				closeButton:true,
				closedHandler:function(){
					problems.next();
				},
		...
		function(){
			var w = $("#input")[0].contentWindow;
			w.$("#input").instruct({
				string:"この問題では、プログラムを変更することはできません。<br>入力データを適切にセットして、<span style='color:#ff0000'>8</span>と出力に表示されるようにしてください。",
				closeButton:true,
			});
