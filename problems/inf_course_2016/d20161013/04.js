...
"}});
		...
		function(){
			...
			w.$("#hints").instruct({
				string:"ここにヒントがあります。またヒントは、追加説明にもなっているので、問題が解けた人も画面上の問題番号をクリックして前の問題に戻って、追加説明を見ても良いです。",
				closeButton:true,
				closedHandler:function(){
					$("#code")[0].contentWindow.$("#run").css("pointer-events","auto");
					$("#code")[0].contentWindow.$("#runInterval").css("pointer-events","auto");
		...
		function(){
			$("#problemsPanel").instruct({
				string:"ここの数字をクリックすると、すでに解いた問題に戻ることができます。また今やっている問題で途中まで書いたものは自動的に保存されて残っているので、戻ってきたときにやり直す必要はありません。",
				closeButton:true,
				closedHandler:function(){
					$("#code")[0].contentWindow.$("#run").css("pointer-events","auto");
					$("#code")[0].contentWindow.$("#runInterval").css("pointer-events","auto");
