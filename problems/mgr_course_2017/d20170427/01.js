new function(){
	...
	var self_n = problems.problems.push([
		function(){
			...
			plib.setExpectedOutputs(["出力<br>3<br>5"]);
...
\/\/ print();は、()内のものを出力します。\n\
\/\/ 数字はそのまま出力されます。\n\
\/\/ 英字からはじまるものは、変数といい、その変数に入っている数字が出力されます。\n\
\/\/ \n\
print(3);\n\
print(a);\n\
"}});
		...
		function(){
			...
			w.$("#input").instruct({
				string:"ここは、プログラムの入力データです。aという変数に5が代入されています。",
				closeButton:true,
		...
		function(){
			var w = $("#output")[0].contentWindow;
			w.$("#inputPanel").instruct({
				string:"複数の出力は、改行かスペースを間に入れてください。",
				closeButton:true,
				closedHandler:function(){
					problems.next();
				},
