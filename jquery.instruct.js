...
(function($){
	var closedHandlers = [];
	$.fn.clearInstruct = function(){
		$('.helpString').remove();
	};
	$.fn.instruct = function(options){
		var $ = window.top.$;
		$('.helpString').remove();
		var opt = $.extend({
...
//console.log("2 size = "+Object.keys(closedHandlers).length);
		...
		var help = $(
			'<div id="'+id+'" class="helpString" style="z-index:9999;position:absolute;">'+
				'<div style="background:mistyrose;border:solid black;padding:8px;">'+
					opt.beforeTag+
					'<div style="color:black;font-size:'+opt.font_size+';float:left;">'+
						opt.string+
					'</div>'+
					opt.afterTag+
...
//		window.top.$("body").append(help[0]);
//		var jq = window.top.$;
//console.log("before ",pos);
		var curWindow = window;
		while(curWindow!==window.top){
			var iframe = findIframe(curWindow.parent.document,curWindow);
			pos.top += window.parent.$(iframe).offset().top;
			pos.left += window.parent.$(iframe).offset().left;
			curWindow = curWindow.parent;
		}
//console.log("after  ",pos);
		var downAni = (pos.top-$('#'+id).height()+opt.offsetY)>0;
		if(downAni === false){
			$('#'+id+' > img').remove();
		...
		var y = downAni ?
			(pos.top-$('#'+id).height()+opt.offsetY):
			(pos.top+$(this).height()+opt.offsetBottom+opt.offsetY);
		help.animate({
			'top':y,
		},1500,"easeInOutExpo");
	};

	function findIframe(doc,w){
		var iframes = doc.getElementsByTagName("iframe");
		for(var frm in iframes){
			if(iframes[frm].contentWindow===w){
				return iframes[frm];
			}
		}
		return null;
